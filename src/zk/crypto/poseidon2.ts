...
type Poseidon2State = [bigint, bigint, bigint, bigint];

/**
 * Apply the raw permutation only to canonical field elements and canonicalize
 * its output before it is reused by the sponge.
 */
function permuteFr(state: Poseidon2State): Poseidon2State {
  const input: Poseidon2State = [
    frMod(state[0]),
    frMod(state[1]),
    frMod(state[2]),
    frMod(state[3]),
  ...
  if (!Array.isArray(output) || output.length !== 4) {
    throw new Error("Poseidon2 permutation returned an invalid state");
  }
...
export function sponge(inputs: bigint[]): bigint {
  ...
  let state: Poseidon2State = [0n, 0n, 0n, iv];

  const full = Math.floor(m / 3);
  for (let i = 0; i < full; i++) {
    state[0] = frAdd(state[0], frMod(inputs[i * 3]!));
    state[1] = frAdd(state[1], frMod(inputs[i * 3 + 1]!));
    state[2] = frAdd(state[2], frMod(inputs[i * 3 + 2]!));
    state = permuteFr(state);
  }

  const remainder = m - full * 3;
  if (remainder !== 0) {
    for (let i = 0; i < remainder; i++) {
      state[i] = frAdd(state[i]!, frMod(inputs[full * 3 + i]!));
    }
    state = permuteFr(state);
  }
...
export function poseidonWithDomain(d: bigint, inputs: bigint[]): bigint {
  return sponge([frMod(d), ...inputs.map(frMod)]);
}
...
export function spongeSqueeze2(d: bigint, sx: bigint, sigma: bigint): [bigint, bigint] {
  ...
  const state = permuteFr([frMod(d), frMod(sx), frMod(sigma), iv]);
