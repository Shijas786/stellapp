...
export interface WithdrawParams {
  ...
  r: bigint;
  /** Expected on-chain spendable commitment opened by `v` and `r`. */
  expectedCSpend: Point;
  /** Public withdrawal amount `a` (0 ≤ a ≤ v). */
  amount: bigint;
  /** Sender's auditor key `K_aud_s` (from the auditor registry). */
  kAudS: Point;
  /** Optional fixed non-zero salt (defaults to a fresh random scalar). */
  sigma?: bigint;
...
export function buildWithdrawWitness(p: WithdrawParams): WithdrawWitness {
  const { keys, v, r, amount, kAudS, expectedCSpend } = p;
  ...
  if (!pointsEqual(cSpend, expectedCSpend)) {
    throw new Error(
      "current spendable opening does not match the expected commitment",
    );
  }

  const sigma = p.sigma ?? randomScalar();
  if (sigma === 0n) {
    throw new Error("withdraw salt must be non-zero");
  }

  const rE = p.rE ?? randomScalar();
  if (rE === 0n) {
    throw new Error("withdraw ephemeral scalar must be non-zero");
  }

  const rNew = deriveSpendR(keys.vk, sigma);
  if (rNew === 0n) {
    throw new Error("derived spendable blinding factor must be non-zero");
  }
