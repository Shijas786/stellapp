...
export interface TransferParams {
  ...
  v: bigint;
  r: bigint;
  /** Expected current sender spendable commitment from trusted state. */
  cSpend: Point;
...
export function buildTransferWitness(p: TransferParams): TransferWitness {
  const { keys, v, r, cSpend: expectedCSpend, amount, pvkB, kAudR, kAudS } = p;
  ...
  // Explicitly enforce conservation: the sender's remaining balance plus the
  // recipient's incoming amount must equal the sender's original balance.
  if (vNew + amount !== v) {
    throw new Error("transfer balance conservation check failed");
  }

  // Verify the supplied opening against the trusted current commitment before
  // including either value in the witness.
  const cSpend = commit(v, r);
  if (!pointsEqual(cSpend, expectedCSpend)) {
    throw new Error("sender spendable commitment does not match its opening");
  }
  ...
  const inputs: NoirInputs = {
    ...
    v_tx: fieldIn(amount),
    r_e: fieldIn(rE),
    ...pointIn("c_spend", expectedCSpend),
    ...pointIn("y", keys.Y),
    ...pointIn("pvk_b", pvkB),
