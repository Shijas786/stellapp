...
export async function hybridFetchEvents(
  ...
  if (indexer && rpcOldest > 0 && next < rpcOldest) {
    ...
    try {
      ...
      if (msg.includes("range") || msg.includes("oldest") || msg.includes("400")) {
        console.warn(
          `[ctd] Stored cursor has aged out of the RPC retention window. ` +
            `Falling back to cold start from retention floor.`,
        );
        const oldest = await rpcOldestLedger(client);
        const floor = oldest > 0 ? oldest + 1 : opts.fromLedger;
